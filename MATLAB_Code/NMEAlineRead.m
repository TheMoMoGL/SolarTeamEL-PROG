...
%                   $GPRMC  Global positioning system fixed data
...
switch case_t
    ...
    case 3 %% GPGSA: Read Procision and fix quality information
        ...
        case 5 %% GPRMC Read global positioning system fixed data
        ...
        if(isempty(t_time))
            ...
            data.UtcTime  = datestr((datenum(t_time,'HHMMSS')),'HH:MM:SS') ;% datenum(t_time,'HHMMSS') - ...
                %floor(datenum(t_time,'HHMMSS'));
        end
        clear t_time;
        
        %next data field is the status char we jump over it
        
        %next data field is the lat
        t_lat  =  fields(4,1:end);
        data.latitude  =  ...
            str2double(t_lat(1:2)) + (str2double(t_lat(3:end))/60);
        t_latDir = fields(5,1:end);
        if(t_latDir  ==  'S')
            data.latitude  =  data.latitude  *  -1;
        end
        clear t_lat t_latDir;
        ...
        if(isempty(t_gspeed))
            ...
            data.groundspeed.kph  = str2double(t_gspeed) * kphFactor;
        end
        ...
        %fix and mode info in fields 13
        
        t_mode = fields(13,1:end);
        switch t_mode(1)
            case 'N'
                data.fixmode='Not Fixed';
            case 'A'
                data.fixmode='Autonomous GNSS Fix';
            case 'D'
                data.fixmode='Differential GNSS Fix';
            case 'E'
                data.fixmode='Estimated/Dead Reckoning Fix';
            otherwise
                data.fixmode=NaN;
        end
        clear t_mode
