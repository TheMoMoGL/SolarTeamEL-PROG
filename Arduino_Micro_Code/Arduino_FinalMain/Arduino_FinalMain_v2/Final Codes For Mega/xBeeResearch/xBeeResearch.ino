...
const uint16_t ThrottlePin = 46;           // Digital pin 4  | Digital pin 46      - Th_out
const uint16_t BrakePin = 4;
const uint16_t SpeedPin= 2;               // New part for the speed update.
...
void setupPIDandLCD()
{
  ...
  pinMode(SpeedPin,INPUT);        // New update for the speed update system...
...
void getSpeed()
{
  ...
  if (millis() - lastmillis == 1000){ //Uptade every one second, this will be equal to reading frecuency (Hz).
    ...
    rpm = (half_revolutions/48) * 60; // Convert frecuency to RPM, note: this works for one interruption per full rotation. For two interrups per full rotation use half_revolutions * 30.
  
    half_revolutions = 0; // Restart the RPM counter
    lastmillis = millis(); // Uptade lasmillis
    attachInterrupt(0, rpm_fun, FALLING); //enable interrupt
    }
  Speed=(2*(3.14)*0.254*rpm*60)/1000 //Converts RPM to Km/h
...
*    orginates from the possible deactivation from the break, if someone brakes
...
void logData()
{
	...
	if (Delay2 - Delay1 > LoggingTime)
	{
		...
		char buffer[456];
		int num_char = sprintf(buffer, "%d ms,    %d,        %d,      %d,       %d,      %d,      %d,       %d\n", (int)timeStamp, (int)LogMsg.Velocity, (int)LogMsg.Throttle, (int)LogMsg.Pvalue, (int)LogMsg.Ivalue, LogMsg.Dvalue, (int)LogMsg.CruiseControlOn, (int)LogMsg.CANmsg1);
...
void serialEvent2()
{
	...
	while (HardSerial_ArduinoToArduino->available())
	{
